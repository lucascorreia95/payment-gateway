...
import (
	...
	"github.com/lucascorreia95/go-gateway/internal/dto"
)
...
func (s *InvoiceService) GetByID(id, apiKey string) (*dto.InvoiceOutput, error) {
	...
	accountOut, err := s.accountService.FindByAPIKey(apiKey)
	if err != nil {
		return nil, err
	}

	if invoice.AccountID != accountOut.ID {
...
func (s *InvoiceService) ListByAccount(accountID string) ([]*dto.InvoiceOutput, error) {
	...
	for i, invoice := range invoices {
		result := dto.FromInvoice(invoice)
		output[i] = &result
	}
